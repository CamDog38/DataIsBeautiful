...
class SnowflakeClient {
   ...
   */
  async getGoogleAdsData(schemaName: string, year?: number): Promise<Partial<AggregatedAdsData>> {
   ...
   */
  async getGoogleAdsSearchTerms(schemaName: string, year?: number): Promise<{
    searchTerm: string;
    clicks: number;
    conversions: number;
    conversionValue: number;
   ...
   */
  async getGoogleAdsHourlyStats(schemaName: string, year?: number): Promise<{
    dayOfWeek: number;
    hour: number;
    impressions: number;
    clicks: number;
   ...
   */
  async getGoogleAdsDeviceStats(schemaName: string, year?: number): Promise<{
    device: string;
    spend: number;
    impressions: number;
    clicks: number;
   ...
   */
  async getGoogleAdsMonthlyPerformance(schemaName: string, year?: number): Promise<{
    monthStart: string;
    spend: number;
    clicks: number;
    conversions: number;
   ...
   */
  async getGoogleAdsTopCampaigns(schemaName: string, year?: number): Promise<{
    campaignId: string;
    campaignName: string;
    impressions: number;
    clicks: number;
   ...
   */
  async getMetaAdsData(schemaName: string, year?: number): Promise<Partial<AggregatedAdsData>> {
    const targetYear = year || new Date().getFullYear();
    const db = "FIVETRAN_DATABASE";
    
    // Meta Ads tables synced by Fivetran
    const sql = `
      SELECT 
        SUM(IMPRESSIONS) as TOTAL_IMPRESSIONS,
        SUM(CLICKS) as TOTAL_CLICKS,
        SUM(SPEND) as TOTAL_SPEND,
        SUM(ACTIONS_VALUE) as TOTAL_CONVERSIONS,
        SUM(PURCHASE_VALUE) as TOTAL_REVENUE
      FROM ${db}.${schemaName}.ADS_INSIGHTS
      WHERE YEAR(DATE_START) = ${targetYear}
    `;
    
    console.log("[Snowflake] Meta Ads query:", sql);
   ...
   */
  async getCampaigns(schemaName: string, platform: string, year?: number): Promise<CampaignData[]> {
    const targetYear = year || new Date().getFullYear();
    ...
    switch (platform) {
      ...
      case "facebook_ads":
        sql = `
          SELECT 
            ...
            SUM(i.ACTIONS_VALUE) as CONVERSIONS,
            SUM(i.PURCHASE_VALUE) as REVENUE
          FROM FIVETRAN_DATABASE.${schemaName}.CAMPAIGN c
          JOIN FIVETRAN_DATABASE.${schemaName}.ADS_INSIGHTS i ON c.ID = i.CAMPAIGN_ID
          WHERE YEAR(i.DATE_START) = ${targetYear}
      ...
      case "linkedin_ads":
        sql = `
          ...
          WHERE YEAR(a.START_AT) = ${targetYear}
          GROUP BY c.ID, c.NAME, c.STATUS
          ORDER BY SPEND DESC
          LIMIT 20
        `;
