...
function normalizeDevice(raw: string): string {
  ...
  if (s.includes("TV") || s.includes("CONNECTED")) return "CONNECTED_TV";
  if (!s) return "UNKNOWN";
  return s.replace(/\s+/g, "_");
}

export function MetaAdsDeviceBreakdownSlide({ slide }: { slide: Slide }) {
  ...
  const aggregatedByDevice = devices.reduce<Record<string, DeviceData>>((acc, d) => {
    const key = normalizeDevice(d.device);
  ...
  const normalized = Object.values(aggregatedByDevice);

  const total = normalized.reduce((sum, d) => sum + (d[metric] || 0), 0);
  const sortedDevices = [...normalized].sort((a, b) => (b[metric] || 0) - (a[metric] || 0));
  ...
  return (
    <div className="relative flex h-full w-full flex-col px-8 py-6">
          ...
          {segments.map((seg, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm font-medium text-white">{formatDeviceName(seg.device)}</div>
      ...
      {segments.length > 0 && (
        <motion.div
          className="text-center mt-4 py-2 px-4 bg-gradient-to-r from-blue-500/20 to-indigo-500/20 rounded-lg"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
