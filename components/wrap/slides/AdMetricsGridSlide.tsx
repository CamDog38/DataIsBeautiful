...
interface MetricsPayload {
  impressions: number;
  clicks: number;
  conversions: number; // Treated as total results
  ctr: number;
  cpc: number;
  cpa: number; // Treated as CPR (cost per result)
  cpm: number;
  currency?: string; // ISO code like USD, ZAR
}

export function AdMetricsGridSlide({ slide }: { slide: Slide }) {
  const payload = slide.payload as MetricsPayload;
  const { impressions, clicks, conversions, ctr, cpc, cpa, cpm, currency = "USD" } = payload;
  ...
  const metrics = [
    ...
    { label: "Results", value: formatNumber(conversions), icon: "🎯", color: "from-emerald-400 to-teal-400" },
    ...
    { label: "CPR", value: `${symbol}${cpa.toFixed(2)}`, icon: "🏷️", color: "from-indigo-400 to-violet-400" },
  ];
  ...
  return (
    <div className="relative flex h-full w-full flex-col items-center justify-center px-10">
      {/* Background */}
      <motion.div
        className="absolute inset-0 opacity-40 bg-[radial-gradient(circle_at_30%_30%,rgba(59,130,246,0.3),transparent_50%),radial-gradient(circle_at_70%_70%,rgba(168,85,247,0.3),transparent_50%)]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.4 }}
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        >
          ...
          <span>→</span>
          <span className="text-emerald-400">{formatNumber(conversions)} results</span>
        </motion.div>
      </div>
    </div>
  );
